# Bylda — Website V2 Audit & Changelog
...
First-time visitors could not tell, within 5 seconds, **what Launchpad Bylda is,
what Bylda is, how they connect, or why they matter.** V2 makes that the
spine of the experience.

## Positioning made explicit everywhere
- **Launchpad Bylda** = *Build the business* (Idea → Validate → Build → Launch):
  Validation, Roadmaps, Tasks, AI Mentors, Progress Tracking.
- **Bylda** = *Automate & scale* (Operate → Automate → Scale):
...
The thin Launchpad/Bylda dropdowns became **mega-menus** with iconned product
links, descriptions, a live preview panel, hover-to-open, and brand theming
(violet = Launchpad Bylda, magenta = Bylda). Built by a single shared script
(`bylda-nav.js`) that self-injects its CSS, so it renders identically on all
...
**Bylda**. The Launch Sequence phases are tagged + tinted by owning product.
...
| File | Purpose |
|------|---------|
| `bylda-v2.css` | Ecosystem section + homepage mega-menu + Launch Sequence tints |
| `bylda-v2.js` | Engine↔journey interaction + Launch Sequence tagging |
| `bylda-nav.js` | Shared mega-menu builder (self-injecting CSS), all pages |
| `blog-progress.js` | Reading-progress bar for article pages |
| `index.html`, `about.html`, `blog/index.html` | Section markup + asset wiring |
| 109 pages | `<script src="/bylda-nav.js">` injected; 25 posts also get `blog-progress.js` |
...
- Bring the ecosystem visual to the Launchpad Bylda and Bylda product pages.
